...
"""
"""
def load_wav(full_path):  # i used this format to generate all the 1000 mels used
    audio, sampling_rate = librosa.load(full_path, sr=22050)  # resamples and bypasses header issues 
    return audio, sampling_rate
"""

def load_wav(full_path):
    # Read wav directly with soundfile
    audio, sampling_rate = sf.read(full_path)
...
def clean_filename(x):
    fname = x.split('|')[0].strip()
    return fname if fname.endswith('.wav') else fname + '.wav'


def get_dataset_filelist(a):
    with open(a.input_training_file, 'r', encoding='utf-8') as fi:
        training_files = [os.path.join(a.input_wavs_dir, clean_filename(x))
                          for x in fi.read().split('\n') if len(x) > 0]

    with open(a.input_validation_file, 'r', encoding='utf-8') as fi:
        validation_files = [os.path.join(a.input_wavs_dir, clean_filename(x))
                            for x in fi.read().split('\n') if len(x) > 0]
